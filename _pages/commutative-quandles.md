...
**Example.** Given a nonempty set \\(X\\), consider the affine hull \\(H_X\\) of \\(X\\) as a subset of the free \\(k\\)-module \\(k[X]\\) generated by \\(X\\): \\[H_X:= \left\\{\sum^n_{i=1}\lambda_i x_i\mid n\geq 1, \lambda_i\in k, x_i\in X, \sum^n_{i=1}\lambda_i=1 \right\\}\subset k[X].\\] Define a quandle operation on \\(H_X\\) by averaging: \\[v\ast w:=\frac{v+w}{2}.\\] Then \\(H_X\\) is a subquandle of the averaging quandle \\( k[X]_{\mathrm{avg}}\\). We denote this subquandle by \\(\mathrm{Free}_X\\); if \\(\\# H=n<\infty\\), then we denote \\(\mathrm{Free}_n:=\mathrm{Free}_X\\). Later, we show that \\(\mathrm{Free}_X\\) is isomorphic to the free commutative quandle generated by \\(X\\), which justifies the notation. Note that \\(\mathrm{Free}_0\\) is the empty quandle.

**Lemma.** _For all \\(n\geq 1\\), the quandle \\(\mathrm{Free}\_n\\) is isomorphic to the averaging quandle \\( k[x\_1,\dots,x\_{n-1}]\_{\mathrm{avg}}\\)._

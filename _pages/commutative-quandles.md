...
**Example.** Given a nonempty set \\(X\\), consider the affine hull \\(H_X\\) of \\(X\\) as a subset of the free \\(k\\)-module \\(k[X]\\) generated by \\(X\\): \\[H_X:= \left\\{\sum^n_{i=1}\lambda_i x_i\Big\mid n\geq 1,\, \lambda_i\in k,\, x_i\in X,\, \sum^n_{i=1}\lambda_i=1 \right\\}\subset k[X].\\] Define a quandle operation on \\(H_X\\) by averaging: \\[v\ast w:=\frac{v+w}{2}.\\] Then \\(H_X\\) is a subquandle of the averaging quandle \\( k[X]_{\mathrm{avg}}\\). We denote this subquandle by \\(\mathrm{Free}_X\\); if \\(\\# H=n<\infty\\), then we denote \\(\mathrm{Free}_n:=\mathrm{Free}_X\\). (Later, we show that \\(\mathrm{Free}_X\\) is isomorphic to the free commutative quandle generated by \\(X\\), which justifies the notation.) Note that \\(\mathrm{Free}_0\\) is the empty quandle.

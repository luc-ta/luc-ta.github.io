...
**Example 3.4.** Given a nonempty set \\(X\\), consider the affine hull \\(H_X\\) of \\(X\\) as a subset of the free \\(k\\)-module \\(k^{\oplus X}\\) generated by \\(X\\): \\[H_X:= \left\\{\sum^n_{i=1}\lambda_i x_i: n\geq 1,\, \lambda_i\in k,\, x_i\in X,\, \sum^n_{i=1}\lambda_i=1 \right\\}\subset k^{\oplus X}.\\] Then \\(H_X\\) is a subquandle of the averaging quandle \\( k^{\oplus X}_{\mathrm{avg}}\\). 
To write \\(H_X\\) itself as an averaging quandle, fix a basepoint \\(x_0\in X\\), and consider the map

$$
    \begin{aligned}
    H_X & \to k^{\oplus X-\{x\}}_{\mathrm{avg}} \\

...
Now, we prove that \\(R\\) is McCoy. Let \\(f,g\in R[x]\cong M_2(k[x])\\) be nonzero elements, considered as \\(2\times 2\\) matrices with polynomial entries \\(f_{ij},g_{ij}\in k[x]\\). Then the equation \\(fg=0\\) means that the polynomials 
    \\[f_{11}g_{11},\quad \varphi:= f_{11}g_{12}+f_{12}g_{22},\quad \psi:= f_{21}g_{11}+f_{22}g_{21},\quad \text{and}\quad f_{22}g_{22}\\] in \\(k[x]\\) all vanish. Since \\(k[x]\\) is an integral domain, we must have \\(f_{11}=0\\) or \\(g_{11}=0\\), and \\(f_{22}=0\\) or \\(g_{22}=0\\). 
